...
export default function AnalysisContent({
  ...
  const [ruleBasedFallback, setRuleBasedFallback] = useState(false);
  ...
  useEffect(() => {
    ...
    if (cached) {
      try {
        ...
        setRuleBasedFallback(parsed.ruleBasedFallback ?? false);
    ...
    <div className="flex flex-col gap-4">
      {ruleBasedFallback && (
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
          ...
          <span>
            <strong>Automated scan</strong> — our AI was temporarily unavailable, so this analysis was generated by our rule-based scanner. Key terms and red flags are detected automatically, but the summary and suggestions may be less detailed than usual. You can{" "}
            <Link href="/upload" className="underline font-medium">re-upload</Link> to get the full AI analysis.
          </span>
        </div>
      )}
